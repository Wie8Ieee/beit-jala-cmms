...
export default function MonthlyMaintenanceEvaluationPage() {
  ...
  const { hasPermission } = useAuth(); const canEdit = hasPermission("edit_reports");
  ...
  const automaticCountKeys = new Set(["totalPmActivities", "completedPmOnTime", "totalCorrectiveRequests", "unclosedCorrectiveRequests", "completedCorrectiveRequests", "totalExternalActivities"]);
  const input = (key: string, label: string, type = "text") => <div><label className="mb-1 block font-medium">{label}</label><Input type={type} disabled={!canEdit} value={String(form[key] ?? "")} onChange={(e) => { set(key, type === "number" ? Number(e.target.value) : e.target.value); if (automaticCountKeys.has(key)) set(`${key}IsOverride`, 1); }} />{automaticCountKeys.has(key) && <p className="mt-1 text-xs text-muted-foreground">محسوب تلقائياً من السجلات، ويمكن تعديله يدوياً.</p>}</div>;
  ...
  return <main className="mx-auto max-w-5xl space-y-5 pb-10" dir="rtl">
    ...
    {query.isLoading ? <p className="text-center text-muted-foreground">جارٍ تحميل التقرير…</p> : <>
      <section className="rounded-xl border bg-card p-5 space-y-5"><h2 className="text-lg font-bold">تقييم تنفيذ أعمال الصيانة</h2><p className="text-sm text-muted-foreground">الأعداد محسوبة تلقائيًا من الخطة الشهرية وسجلات الصيانة الوقائية المنفذة في نفس الشهر، ويمكن تعديلها يدوياً.</p><div><p className="mb-2 font-medium">(1) أعمال الصيانة الوقائية المجدولة في هذا الشهر والتي لم يتم تنفيذها</p>{rowsTable("delayedActivities", [{ key: "activity", label: "نشاطات أعمال الصيانة" }, { key: "reason", label: "سبب التأخير" }], { activity: String(form.delayedActivities ?? ""), reason: String(form.delayReason ?? "") }, () => set("delayReason", ""))}</div><div><p className="mb-2 font-medium">(2) هل تم جدولة جميع هذه النشاطات في برنامج الصيانة الوقائية الشهرية للشهر القادم؟</p><YesNo value={String(form.followUpIncluded ?? "")} disabled={!canEdit} onChange={(v) => set("followUpIncluded", v)} /></div><div className="grid gap-4 md:grid-cols-2">{input("completedPmOnTime", "عدد نشاطات الصيانة الوقائية التي طُبقت (Achieved)", "number")}{input("totalPmActivities", "عدد النشاطات التي خُطّط لها (Planned)", "number")}</div>{formula("completedPmOnTime", "totalPmActivities", "(3) النسبة المئوية لإنجاز برنامج الصيانة الوقائية الشهرية")}</section>
      ...
      <section className="rounded-xl border bg-card p-5 space-y-5">{input("preparedBy", "إعداد")}{input("preparedDate", "تاريخ الإعداد", "date")}{input("engineeringManagerSignature", "اسم مدير دائرة الهندسة / توقيع بديل")}<div><label className="mb-1 block font-medium">تاريخ توقيع مدير دائرة الهندسة</label><div className="flex gap-2"><Input type="date" disabled={!canEdit} value={String(form.engineeringManagerDate ?? "")} onChange={(event) => set("engineeringManagerDate", event.target.value)} />{canEdit && <Button type="button" variant="outline" onClick={() => set("engineeringManagerDate", new Date().toISOString().slice(0, 10))}>تاريخ اليوم</Button>}</div></div>{Number(form.id) > 0 ? <><ElectronicSignatureField documentType="MONTHLY_MAINTENANCE_EVALUATION" documentId={Number(form.id)} fieldName="prepared_by" label="التوقيع الإلكتروني للمُعدّ" /><ElectronicSignatureField documentType="MONTHLY_MAINTENANCE_EVALUATION" documentId={Number(form.id)} fieldName="engineering_manager" label="التوقيع الإلكتروني لمدير دائرة الهندسة" /></> : <p className="rounded-md border border-dashed p-3 text-sm text-muted-foreground">احفظ التقرير أولاً، ثم يمكن للمُعدّ ومدير دائرة الهندسة المعتمدين توقيعه إلكترونياً.</p>}</section>
    </>}
  </main>;
}
