...
export default function MonthlyMaintenanceEvaluationPage() {
  ...
  return <main className="mx-auto max-w-5xl space-y-5 pb-10" dir="rtl">
    ...
    {query.isLoading ? <p className="text-center text-muted-foreground">جارٍ تحميل التقرير…</p> : <>
      <section className="rounded-xl border bg-card p-5 space-y-5"><h2 className="text-lg font-bold">تقييم تنفيذ أعمال الصيانة</h2><p className="text-sm text-muted-foreground">الأعداد محسوبة تلقائيًا من الخطة الشهرية وسجلات الصيانة الوقائية المنفذة في نفس الشهر.</p><div><p className="mb-2 font-medium">(1) أعمال الصيانة الوقائية المجدولة في هذا الشهر والتي لم يتم تنفيذها</p>{rowsTable("delayedActivities", [{ key: "activity", label: "نشاطات أعمال الصيانة" }, { key: "reason", label: "سبب التأخير" }], { activity: String(form.delayedActivities ?? ""), reason: String(form.delayReason ?? "") }, () => set("delayReason", ""))}</div><div><p className="mb-2 font-medium">(2) هل تم جدولة جميع هذه النشاطات في برنامج الصيانة الوقائية الشهرية للشهر القادم؟</p><YesNo value={String(form.followUpIncluded ?? "")} disabled={!canEdit} onChange={(v) => set("followUpIncluded", v)} /></div><div className="grid gap-4 md:grid-cols-2"><div><label className="mb-1 block font-medium">عدد نشاطات الصيانة الوقائية التي طُبقت (Achieved)</label><Input type="number" disabled value={Number(form.completedPmOnTime ?? 0)} /></div><div><label className="mb-1 block font-medium">عدد النشاطات التي خُطّط لها (Planned)</label><Input type="number" disabled value={Number(form.totalPmActivities ?? 0)} /></div></div>{formula("completedPmOnTime", "totalPmActivities", "(3) النسبة المئوية لإنجاز برنامج الصيانة الوقائية الشهرية")}</section>
