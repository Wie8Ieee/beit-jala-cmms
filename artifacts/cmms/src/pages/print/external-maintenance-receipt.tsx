...
export default function ExternalMaintenanceReceiptPrintPage({ params }: { params: { id: string } }) {
  ...
  return <PrintLayout title="External Maintenance Work Receipt — Official Print"><PrintPage><div dir="rtl" className="official-print-external-receipt"><div className="official-print-external-receipt-sheet">
    ...
    <div className="official-print-external-receipt-content">
      <div className="official-print-receipt-top-row"><span>طلب صيانة رقم: <Fill><bdi dir="ltr">{request.requestReportNumber}</bdi></Fill></span><span>نوع أعمال الصيانة: <Fill>{receipt.maintenanceType}</Fill></span></div>
      <div className="official-print-receipt-top-row"><span>القسم الطالب للصيانة: <Fill>{receipt.requestingDepartment}</Fill></span><span>التاريخ: <Fill><bdi dir="ltr">{formatDate(receipt.receiptDate)}</bdi></Fill></span></div>
      <div className="official-print-receipt-performer">الجهة المنفذة للعمل: <Fill>{receipt.performingEntity}</Fill></div>
      <section className="official-print-receipt-ruled-section"><div>تقرير استلام أعمال الصيانة:</div><DottedWritingLines value={receipt.workAcceptanceReport} count={6} /></section>
      <section className="official-print-receipt-ruled-section official-print-receipt-rejection"><div>سبب الرفض في حالة وجود خطأ في العمل:</div><DottedWritingLines value={receipt.workFailureCause} count={4} /></section>
      <div className="official-print-receipt-examiner"><div>اسم الفاحص: <Fill>{receipt.examinerName}</Fill></div><div>توقيع الفاحص: <Fill>{receipt.examinerSignature}</Fill></div></div>
    </div>
