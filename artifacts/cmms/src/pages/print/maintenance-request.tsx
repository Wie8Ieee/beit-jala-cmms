...
function SignatureLine({ label, signature, fallback }: { label: string; signature?: DrawnSignature; fallback?: string | null }) {
  const signatureData = signature?.signatureData || (fallback?.startsWith("data:image/") ? fallback : null);
  return <span className="official-print-mr-signature-line">{label}: {signatureData ? <img src={signatureData} className="official-print-form-signature" alt={label} /> : <DottedLine />}</span>;
}

function RuledWritingArea({ text, lines, className = "", solid = false, prefix, trailingBlankLine = false }: { text?: string | null; lines: number; className?: string; solid?: boolean; prefix?: string; trailingBlankLine?: boolean }) {
  // A textarea stores Enter as a newline. Render each entered line inside its
  // own ruled row so its baseline always matches the next printed writing line.
  const enteredLines = (text ?? "").replace(/\r\n/g, "\n").split("\n");
  const totalLines = Math.max(lines + 1, enteredLines.length + (trailingBlankLine ? 1 : 0));
  return (
    <div className={`official-print-mr-ruled-area ${solid ? "official-print-mr-solid-lines" : ""} ${className}`}>
      {Array.from({ length: totalLines }).map((_, index) => (
        <div key={index} className="official-print-mr-ruled-value">
          {index === 0 && prefix ? <strong className="official-print-mr-ruled-prefix">{prefix}</strong> : null}
          {enteredLines[index] || "\u00a0"}
        </div>
      ))}
    </div>
...
export default function MaintenanceRequestPrintPage({ params }: { params: { id: string } }) {
  ...
  const signature = (field: string) => requestSignatures.find((item) => item.fieldName === field);
  ...
  return <PrintLayout title="Maintenance Request - Official Print"><PrintPage><div dir="rtl" className="official-print-maintenance-request"><div className="official-print-maintenance-request-content">
    ...
    <section className="official-print-mr-section"><h3>نتائج الكشف الأولي (خاص بالصيانة)</h3><RuledWritingArea text={event?.preliminaryCheckResults} lines={3} className="official-print-mr-findings" /><div className="official-print-mr-worktime"><span>وقت بدء العمل المتوقع: من <DottedLine text={event?.expectedWorkTimeFrom || data?.expectedWorkTimeFrom} /> إلى: <DottedLine text={event?.expectedWorkTimeTo || data?.expectedWorkTimeTo} /></span></div><div className="official-print-mr-signature-row official-print-mr-technician-signatures"><SignatureLine label="توقيع فني الصيانة" signature={signature("maintenance_technician")} fallback={event?.maintenanceTechnicianSignature} /><SignatureLine label="توقيع مشرف القسم المعني" signature={signature("concerned_section_supervisor")} fallback={event?.concernedSectionSupervisorSignature} /></div></section>

    <section className="official-print-mr-section"><h3>الإجراءات المتخذة</h3><RuledWritingArea text={event?.actionsTaken} lines={9} className="official-print-mr-actions" /><RuledWritingArea text={event?.remarksRecommendations} lines={1} className="official-print-mr-remarks" prefix="ملاحظات وتوصيات:" trailingBlankLine /></section>

    <table className="official-print-table official-print-mr-staff-table"><thead><tr><th className="w-[8%]">الرقم</th><th>القائم بالعمل</th><th>التوقيع</th></tr></thead><tbody>{staff.map((person, index) => { const staffSignature = signature(`performing_staff_${index + 1}`); return <tr key={index}><td>{person?.no || index + 1}</td><td>{person?.name}</td><td>{staffSignature?.signatureData ? <img src={staffSignature.signatureData} className="official-print-form-signature" alt={`توقيع ${person?.name || index + 1}`} /> : person?.signature}</td></tr>; })}</tbody></table>
    <section className="official-print-mr-closing"><h3>تسليم الماكينة / العمل المنجز</h3><div><SignatureLine label="توقيع مستلم الماكينة/ العمل المنجز" signature={signature("receiver")} fallback={event?.receiverSignature} /><span>التاريخ: <DottedLine text={formatDate(event?.handoverDate)} /></span></div><div><SignatureLine label="توقيع الهندسة" signature={signature("engineering_final")} fallback={event?.engineeringSignature} /><span>التاريخ: <DottedLine text={formatDate(event?.engineeringDate)} /></span></div></section>
