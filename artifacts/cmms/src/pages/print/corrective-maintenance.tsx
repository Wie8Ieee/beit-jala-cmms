...
} from "../maintenance-requests/types";
...
export default function CorrectiveMaintenancePrintPage({
  params,
}: {
  params: { id: string; recordId?: string };
}) {
  const machineId = Number(params.id);
  const historicalRecordId = params.recordId
  ...
  const printEvents: Array<CorrectiveMaintenanceEvent | undefined> = [
    ...events,
    ...Array.from<undefined>({ length: Math.max(0, 3 - events.length) }),
  ];
      ...
      {record && (
        <PrintPage landscape>
          <div dir="rtl" className="official-print-cm-content">
            <table
              dir="ltr"
              className="official-print-table official-print-header-table official-print-cm-header"
            >
              <tbody>
                <tr>
                  <td dir="rtl" className="w-[28%] text-right">
                    <div>
                      رقم الوثيقة: <bdi dir="ltr">{record.documentNumber}</bdi>
                    </div>
                    <div>
                      تاريخ التنفيذ:{" "}
                      <bdi dir="ltr">
                  ...
                  <td dir="rtl" className="w-[44%] text-center font-semibold">
                    ...
                    <div dir="ltr">
                      (Equipment Corrective Maintenance Record)
                    </div>
                  </td>
                  <td dir="rtl" className="w-[28%] text-right">
            ...
            <div className="official-print-cm-machine-details my-4 grid grid-cols-4 gap-4 text-right font-semibold">
              ...
              <div>
                رقم الماكينة: <bdi dir="ltr">{record.machineNumber}</bdi>
              </div>
              <div>
                مكان وجود الماكينة:{" "}
            ...
            <table className="official-print-table official-print-cm-log text-right">
              ...
              <tbody>
                {printEvents.map((event, eventIndex) => {
                  const repairSlots = Array.from(
                    { length: 5 },
                    (_, index) =>
                      event?.repairTimeSlots?.[index] ?? {
                        date: "",
                        from: "",
                        to: "",
                      },
                  ...
                  return (
                    <Fragment key={`event-${eventIndex}`}>
                      ...
                      >
                        ...
                        <td rowSpan={5}>
                          {maintenanceType(
                            event?.maintenanceType ?? event?.priority,
                          )}
                        </td>
                        ...
                        <td>
                          {repairSlots[0].from ||
                            event?.expectedWorkTimeFrom ||
                            ""}
                        </td>
                        <td>
                          {repairSlots[0].to || event?.expectedWorkTimeTo || ""}
                        </td>
                      </tr>
                      {Array.from({ length: 4 }).map((_, rowIndex) => {
                        ...
                        const startsDate =
                          !slot.date ||
                          slot.date !== repairSlots[slotIndex - 1].date;
                        return (
                          <tr
                            key={`event-${eventIndex}-time-${rowIndex}`}
                            className="h-[18px]"
                          >
                            {startsDate && (
                              <td
